...
- [More Than Image Generators: A Science of Problem-Solving using Probability](https://www.youtube.com/watch?v=Fk2I6pa6UeA) - a SoME4 entry that deliberately skips the usual ELBO and fixed-encoder-VAE story and comes at diffusion through score matching and Langevin dynamics instead, following Song and Ermon's [NCSN paper](https://arxiv.org/abs/1907.05600). The argument is that the standard derivation costs a lot and buys very little intuition, and it's hard to argue after watching; sampling an image and rolling a die come out as the same recipe, and the whole thing reads as gradient descent moved from train time to test time. Good counterweight to the Luo tutorial below, which takes exactly the route this one rejects
